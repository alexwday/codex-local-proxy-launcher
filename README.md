...
- Provides a local dashboard for setup values, logs, usage, model mapping, and a `kilo.jsonc` snippet.
...
- `MODEL_OPTIONS`, `MODEL_MAPPING`, `DEFAULT_MODEL`, `STRICT_MODEL_ALLOWLIST`
...
5. Run `./run.sh`.
6. In Kilo Code, configure an OpenAI-compatible custom provider:
   - Base URL: `http://localhost:<PROXY_PORT>/v1`
...
The test suite covers local auth, `/v1/models`, model mapping,
